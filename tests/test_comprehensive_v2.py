...
class TestCase:
    def __init__(self, name: str, note_file: str, guideline_file: str, 
                 acceptable_decisions: Set[str], priority_decision: str = None):
        self.name = name
        self.note_file = note_file
        self.guideline_file = guideline_file
        self.acceptable_decisions = acceptable_decisions  # Set of acceptable decisions
        self.priority_decision = priority_decision  # Preferred decision
        self.result = None
        self.passed = None
        self.error = None
...
class TestSuite:
    ...
    def run_all_tests(self):
        print("="*80)
        print("COMPREHENSIVE PRIOR AUTHORIZATION TEST SUITE V2")
        print("="*80)
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            ...
            try:
                ...
                if actual_decision in test_case.acceptable_decisions:
                    test_case.passed = True
                    passed += 1
                    
                    if actual_decision == test_case.priority_decision:
                        print(f"\n✅ TEST PASSED (Optimal Decision)")
                    else:
                        print(f"\n✅ TEST PASSED (Acceptable Decision)")
                        print(f"   Got: {actual_decision}")
                        print(f"   Preferred: {test_case.priority_decision}")
                else:
                    test_case.passed = False
                    failed += 1
                    print(f"\n❌ TEST FAILED")
                    print(f"   Acceptable: {test_case.acceptable_decisions}")
                    print(f"   Got: {actual_decision}")
                ...
                # Print first 3 reasoning points
                print(f"\n💡 Key Reasoning:")
                for idx, reason in enumerate(result['decision']['reasoning'][:3], 1):
                    reason_preview = reason[:150] + "..." if len(reason) > 150 else reason
                    print(f"   {idx}. {reason_preview}")
        ...
        for idx, test_case in enumerate(self.test_cases, 1):
            ...
            if test_case.result:
                decision = test_case.result['decision']['decision']
                confidence = test_case.result['decision']['confidence_score']
                print(f"      Decision: {decision} ({confidence:.0%} confidence)")
    ...
    def run_test_case(self, test_case: TestCase) -> Dict:
        ...
        # Safe extraction
        diagnoses = note_result['structured_data'].get('diagnoses') or []
        procedures = note_result['structured_data'].get('procedures') or []
        print(f"   📊 Extracted: {len(diagnoses)} diagnoses, {len(procedures)} procedures")
        ...
        criteria_list = guideline_criteria.get('coverage_criteria') or []
        print(f"   📋 {len(criteria_list)} coverage criteria")
...
def create_test_suite():
    ...
    # Has all criteria: documented pain 6+ weeks, MRI confirmed, 8 weeks PT, 
    # NSAID trial, pain 7/10, mechanical symptoms, Grade III tear
    suite.add_test_case(TestCase(
        ...
        acceptable_decisions={"approved"},  # Should be approved
        priority_decision="approved"
    ))
    
    # Test Case 2: Should be DENIED
    # Only 2 weeks of pain (needs 6 weeks), no imaging, acute strain (not covered diagnosis)
    suite.add_test_case(TestCase(
        ...
        acceptable_decisions={"denied"},  # Clear denial
        priority_decision="denied"
    ))
    
    # Test Case 3: Should be DENIED
    # Missing required non-invasive testing (policy requires stress test first)
    suite.add_test_case(TestCase(
        ...
        acceptable_decisions={"denied"},  # Missing required test = denied
        priority_decision="denied"
    ))
    
    # Test Case 4: Should be APPROVED
    # Has diagnosis M51.16, 6 weeks pain, 3 months PT, NSAID trial, MRI confirmed,
    # pain 8/10 documented, procedure CPT 62323 covered
    suite.add_test_case(TestCase(
        name="Lumbar Epidural - Original Case",
        note_file="data/synthetic_notes/sample_note_1.txt",
        guideline_file="data/guidelines/lumbar_injection_guideline.txt",
        acceptable_decisions={"approved"},  # Should be approved with new prompt
        priority_decision="approved"
    ))
...
if __name__ == "__main__":
    print("\n🧪 Starting Comprehensive Test Suite V2...\n")
